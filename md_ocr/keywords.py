...
LLM_REEXTRACT_CONFIG = {
    ...
    3: {
        ...
        7: {
            ...
            从这张报告图片中提取 “前向精子百分率(Progressive motile)” 的检测结果数值。不要输出任何其它内容。
            要求：
                1. 如果没有找到数值则默认数值为 -1
            """,
            ...
            "prompt_uni":
             """
            从这张报告图片中提取 “前向精子百分率(Progressive motile)” 的检测结果单位。不要输出任何其它内容。
            要求：
                1. 如果没有找到单位则默认单位为 ""
            """
        }
    ...
    31: {
        ...
        #     "prompt_val": 
        #     """
        #     从这报告图片中提取 “阴道清洁度” 的检测结果。不要输出任何其它内容。
        #     要求：
        #         1. 如果没有找到检测结果则默认检测结果为 -1
        #     """,

        #     "prompt_uni": 
        #     """
        #     从这报告图片中提取 “阴道清洁度” 的检测结果单位。不要输出任何其它内容。
        #     要求：
        #         1. 如果没有找到单位则默认单位为 ""
        #     """
        # },
        ...
        1: {
            "prompt_val": 
            """
            从这份报告图片中按要求提取 “白细胞” 的检测结果并输出。不要输出任何其它内容。
            要求：
                1.如果检测结果为一个范围则返回一个仅包含两个数字的列表，第一个数字为范围下限，第二个数字为范围上限
                2.如果检测结果为一个数字则直接输出,不要包含列表
                3.如果检测结果为字符则直接输出，不要包含列表
                4.要求3与要求1冲突，以要求1为准
                5.如果没有找到检测结果则默认检测结果为 -1
            """,

            "prompt_uni": 
            """
            从这报告图片中提取 “白细胞” 的检测结果单位。不要输出任何其它内容。
            要求：
                1. 如果没有找到单位则默认单位为 ""
            """
        }
...
CROSS_REPORT_CONFLICT_RULES = [
    ...
    },
    {
        ...
        # 目标指标 index（白带常规第2个指标：红细胞）
        "target_index": 2,

        # 命中这些单位时触发
        "target_units": ["×10^12/L", "10^12/L", "×10/L", "10/L"],
